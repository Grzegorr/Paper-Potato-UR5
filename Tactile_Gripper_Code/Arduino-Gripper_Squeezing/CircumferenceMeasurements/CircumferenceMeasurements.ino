...
void setup() {
  ...
  Sensor_thresh = Sensor_thresh + 10;
...
//run it in reference object
void initial_encoder_value(){
  delay(4000);
  ...
  analogWrite(PWM_A, 0);
  delay(200);
  Serial.println("initial A: " + String(A_count));
  saved_A = A_count;
  delay(8000);
...
void measure_squeeze(){
  ...
  analogWrite(PWM_A, 0);
  delay(200);
  Serial.println("Final A: " + String(A_count));
  int distance_enc = A_count - saved_A;
  Serial.println("Distance_enc: " + String(distance_enc));
  float distance_mm = distance_enc*3.14*8.5/3.0/298.0;
  distance_mm = 220.7 - distance_mm;
  Serial.println("circumference: " + String(distance_mm));
